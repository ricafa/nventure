...
<div
    x-data="{
        ...
        async register() {
            ...
            try {
                await window.Passkeys.register({ nome: this.nome });
                this.nome = '';
                this.showForm = false;
                await $wire.loadPasskeys();
            } catch (e) {
                if (e.constructor?.nome !== 'UserCancelledError') {
...
>
    ...
    <template x-if="supported && showForm">
        <div class="space-y-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 p-4">
            <flux:input
                label="{{ __('Passkey nome') }}"
                x-model="nome"
                placeholder="{{ __('e.g., MacBook Pro, iPhone') }}"
                x-on:keydown.enter.prevent="register()"
                x-ref="passkeyNomeInput"
                x-init="$nextTick(() => $refs.passkeyNomeInput?.focus())"
            />
            <flux:text class="!mt-1">{{ __('Give this passkey a nome to help you identify it later.') }}</flux:text>
            ...
            <div class="flex gap-2">
                <flux:button
                    variant="primary"
                    x-on:click="register()"
                    x-bind:disabled="loading || !nome.trim()"
                >
